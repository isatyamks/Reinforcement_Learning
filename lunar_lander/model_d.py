...
def train_model(Q=None, num_episodes=10000, alpha=0.1, gamma=0.99, 
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 500 == 0:
            mean_reward_500 = np.mean(rewards[-500:])
            print(f"Episode {episode+1}/{num_episodes}, Mean Reward (last 500 episodes): {mean_reward_500:.2f}")
        if (episode + 1) % 1000 == 0:
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        Q = train_model(Q=Q, num_episodes=20000)
        save_model(Q)
    elif args.test:
