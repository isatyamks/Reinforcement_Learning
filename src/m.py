...
class CartPoleEnv:
    ...
    def render(self):
        ...
        # Draw the pole (rod) with a weight at the end
        ...
        # Display live score (number of steps survived)
        score_text = self.font.render(f"Score: {self.steps}", True, (0, 0, 0))
        self.screen.blit(score_text, (10, 10))

        # If game over, display a large GAME OVER message
        if self.game_over:
...
# Main Q-learning loop integrated with the custom Pygame environment and live matplotlib plot
def run(is_training=True, render=False):
    ...
    if is_training:
        ...
        with open(f'cartpole_{tp}.pkl', 'rb') as f:
            q = pickle.load(f)
    ...
    rewards_per_episode = []
    live_mean_rewards = []  
    episodes_axis = []      # X-axis values for the plot (episode numbers)
    i = 0
    ...
    if is_training:
        ...
        # ax.set_title('Live Mean Rewards')
        # ax.legend()
        # plt.show()
    while True:
        state = env.reset()
        ...
        if is_training and i%100==0:
            ...
            live_mean_rewards.append(mean_rewards)
            # line.set_data(episodes_axis, live_mean_rewards)
            # ax.relim()
            # ax.autoscale_view()
            # plt.draw()
            # plt.pause(0.01)
    ...
    if is_training:
        with open(f'cartpole_{tp}.pkl', 'wb') as f:
            pickle.dump(q, f)
    plt.ioff()
    plt.savefig(f'cartpole_rewards_{tp}.png')
